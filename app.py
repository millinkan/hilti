...
from prioritization import (
    ...
    simulate_portfolio_profit, sensitivity_tornado, simulate_rank_stability,
)
...
if current_page == 'Portfolio Overview':
...
elif current_page == 'Risk Simulation':
    # Risk Simulation (Monte Carlo) — whole portfolio or a single project
    st.subheader("Monte Carlo Risk Simulation")
    st.caption("Stress-test outcomes under uncertainty: each project's monthly business value and cost are randomly perturbed thousands of times to reveal the range of possible results.")
    ...
    with rc2:
        bv_std = st.slider("Business Value Variance (±%)", 0, 100, 20, key="risk_bv") / 100.0
    with rc3:
        cost_std = st.slider("Cost Variance (±%)", 0, 100, 10, key="risk_cost") / 100.0
...
elif current_page == 'Robustness':
    st.subheader("Robustness")
    st.caption("How much can we trust the ranking? This page stress-tests our assumptions in two ways: which inputs the result depends on most, and whether the priority order holds up when the numbers are uncertain.")

    # ---------- A) Which assumptions matter most? (sensitivity) ----------
    st.markdown("### Which assumptions matter most?")
    st.caption("If a key input turns out to be off by ±X%, how much does the **total portfolio net profit** move? A longer bar means the portfolio is more sensitive to that assumption — so it deserves the most scrutiny.")
    pct = st.slider("Vary each input by ±%", 1, 30, 10, 1, key="robust_pct") / 100.0
    base_np, rows = sensitivity_tornado(projects, pct=pct)

    labels = [r["driver"] for r in rows][::-1]      # largest swing on top
    lows = [r["low_np"] for r in rows][::-1]
    highs = [r["high_np"] for r in rows][::-1]
    fig_t = go.Figure()
    fig_t.add_trace(go.Bar(
        y=labels, x=[h - l for l, h in zip(lows, highs)], base=lows,
        orientation="h", marker_color="#D2051E", opacity=0.85,
        customdata=[[l, h] for l, h in zip(lows, highs)],
        hovertemplate="<b>%{y}</b><br>Low: %{customdata[0]:,.0f}<br>High: %{customdata[1]:,.0f}<extra></extra>",
    ))
    fig_t.add_vline(x=base_np, line_dash="dash", line_color="white", annotation_text="Base case")
    fig_t.update_layout(title=f"Sensitivity of Total Portfolio Net Profit (±{int(pct*100)}%)",
                        xaxis_title="Total Portfolio Net Profit (CHF)",
                        margin=dict(t=40, b=20, l=20, r=20), height=320)
    st.plotly_chart(fig_t, use_container_width=True)
    st.info(f"**Most influential assumption: {rows[0]['driver']}** — a ±{int(pct*100)}% error here swings total net profit by {fmt_money(rows[0]['swing'])}. Base-case portfolio net profit: {fmt_money(base_np)}.")
    ...
    st.info("💡 **Prototype Phase 2:** Advanced Portfolio Optimization. Compare prioritization methods and execution modes side-by-side under the current constraints.")

    # --- Prioritization method comparison (Point 2) ---------------------
    ...
    for m in ["Composite", "Hilti Value Creation Rating", "ROI"]:
        ...
        tl_m = build_global_timeline(sched_m)
        np_m = float(tl_m["Cumulative Net Profit"].iloc[-1]) if len(tl_m) > 0 else 0.0
    ...
    best_method = df_methods.loc[best_idx, "Method"]
    mc1, mc2 = st.columns([3, 2])
    with mc1:
        fig_m = px.bar(df_methods, x="Method", y="_np", color="Method",
                       title="Total Net Profit by Prioritization Method",
                       labels={"_np": "Total Net Profit (CHF)"},
                       color_discrete_sequence=px.colors.qualitative.Bold)
        fig_m.update_layout(margin=dict(t=40, b=20, l=20, r=20), showlegend=False)
        st.plotly_chart(fig_m, use_container_width=True)
    with mc2:
        disp = df_methods.copy()
        disp["Total Net Profit"] = disp["_np"].map(fmt_money)
        disp["Total Cost"] = disp["Total Cost"].map(fmt_money)
        disp["ROI (%)"] = disp["ROI (%)"].map(lambda v: f"{v:.1f}%")
        disp["Selected"] = disp["Selected"].map(lambda v: f"{v} / {len(projects)}")
        st.dataframe(disp[["Method", "Total Net Profit", "ROI (%)", "Selected", "Timeline (mo)"]],
                     hide_index=True, use_container_width=True)
    st.success(f"🏆 Best method under current settings (by total net profit): **{best_method}** — {fmt_money(df_methods.loc[best_idx, '_np'])}.")
    st.divider()
    ...
    st.subheader("Interdepartmental Alignment & Buy-In (Phase 3)")
